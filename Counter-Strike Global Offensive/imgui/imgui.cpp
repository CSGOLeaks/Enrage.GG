...
ImU32 ImGui::GetColorU32(ImGuiCol idx, float alpha_mul,float alpha_replace)  
{ 
    ImGuiStyle& style = GImGui->Style;
    ImVec4 c = style.Colors[idx]; 
 
	if (alpha_replace > -1.f)
		c.w = alpha_replace;
	else
...
bool ImGui::BeginGroupBox(const char* name, const ImVec2& size_arg)
{
	...
	flags |= (window->Flags & ImGuiWindowFlags_NoMove);  // Inherit the NoMove flag
	...
	ImGui::SetNextWindowSize(size);
	bool ret = ImGui::Begin(name, NULL, flags);
	...
	//window->DC.CursorPos.y += 3;

	//ImGui::SetCursorPosY(window->DC.CursorPos.y + 5);
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags, bool* clicked_back)
{
    ImGuiContext& g = *GImGui;
    const ImGuiStyle& style = g.Style;
    ...
    if (first_begin_of_the_frame)
    {
        ...
        window->WindowBorderSize = (flags & ImGuiWindowFlags_ChildWindow && !(flags & ImGuiWindowFlags_GroupBox)) ? style.ChildBorderSize : ((flags & ImGuiWindowFlags_Popup) && !(flags & ImGuiWindowFlags_Modal)) ? style.PopupBorderSize : style.WindowBorderSize;
        window->WindowPadding = style.WindowPadding;
		if (!(flags & ImGuiWindowFlags_GroupBox) && (flags & ImGuiWindowFlags_ChildWindow) && !(flags & (ImGuiWindowFlags_AlwaysUseWindowPadding | ImGuiWindowFlags_Popup)) && window->WindowBorderSize == 0.0f)
			window->WindowPadding = ImVec2(0.0f, (flags & ImGuiWindowFlags_MenuBar) ? style.WindowPadding.y : 0.0f);
		else if (flags & ImGuiWindowFlags_GroupBox)
			window->WindowPadding.y += 4.f;
        ...
        else
        {
			...
			if (!(flags & ImGuiWindowFlags_NoTitleBar)) {
				window->DrawList->AddRectFilled(title_bar_rect.Min/* - ImVec2(0, 10)*/, title_bar_rect.Max, GetColorU32(window_is_focused ? ImGuiCol_TitleBgActive : ImGuiCol_TitleBg), window_rounding, ImDrawCornerFlags_Top);
			
				if (clicked_back && flags & ImGuiWindowFlags_CoolStyle) {
					RenderTriangle(title_bar_rect.Min + style.FramePadding + ImVec2(2, 4), ImGuiDir_Left, 2.0f);

					//window->DrawList->AddRectFilled(title_bar_rect.Min + style.FramePadding, title_bar_rect.Min + style.FramePadding + ImVec2(20, 28), ImU32(((255 & 0xff) << 24) + ((0 & 0xff) << 16) + ((0 & 0xff) << 8)
					//	+ (255 & 0xff)), window_rounding, ImDrawCornerFlags_Top);

					if (g.HoveredWindow == window && IsMouseHoveringRect(title_bar_rect.Min + style.FramePadding, title_bar_rect.Min + style.FramePadding + ImVec2(20, 28)) && g.IO.MouseClicked[0])
						*clicked_back = true;
				}
			}
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar))
        {
			...
			window->DrawList->AddRectFilled(window->Pos, window->Pos + ImVec2(window->Size.x, text_size.y * 1.5), GetColorU32(ImGuiCol_Border), window_rounding);

			RenderText(window->Pos + ImVec2(g.Style.WindowPadding.x, 2), name, NULL, true);
			...
			window->DC.CursorPos.y += text_size.y * 1.5;
			//}
		}
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (hovered)
    {
        if (!(flags & ImGuiButtonFlags_NoKeyModifiers) || (!g.IO.KeyCtrl && !g.IO.KeyShift && !g.IO.KeyAlt))
        {
            ...
            //    pressed = true;
			if (flags & ImGuiButtonFlags_Repeat && g.ActiveId == id && IsMouseClicked(0, true))
				pressed = true;
        }
    }
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
	...
	if (flags & ImGuiItemFlags_TabButton)
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
    ...
    const float grab_padding = 0.f/*2.0f*/;
    ...
    if (g.ActiveId == id && ImGui::is_input_allowed)
    {
	...
	window->DrawList->AddRectFilledMultiColor(frame_bb.Min, grab_bb.Max, GetColorU32(ImGuiCol_SliderGrabActive), GetColorU32(ImGuiCol_SliderGrabActive), GetColorU32(ImGuiCol_SliderGrab), GetColorU32(ImGuiCol_SliderGrab));

	std::string noob = "%.1f";

	if (decimal_precision <= 9)
		noob[2] = std::to_string(decimal_precision).c_str()[0];

	char value_buf[64];
	const char* value_buf_end = value_buf + ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), noob.c_str(), *v);

	if (flags & ImGuiSliderFlags_RenderValue) 
	{
		const ImVec2 label_size = CalcTextSize(value_buf, value_buf_end, true);

		const auto cur_clipping = window->ClipRect;

		ImGui::PopClipRect();

		auto x_pos = grab_pos - label_size.x * 0.5f;

		if (x_pos + label_size.x > slider_usable_pos_max)
			x_pos = slider_usable_pos_max - label_size.x;

		if (x_pos < frame_bb.Min.x)
			x_pos = frame_bb.Min.x;

		ImVec4 *colors = (ImVec4*)style.Colors;
		const auto text_color = colors[ImGuiCol_Text];
		colors[ImGuiCol_Text] = ImVec4(0.1f, 0.1f, 0.1f, text_color.w);
		RenderText(ImVec2(x_pos + 1.f, frame_bb.Min.y - 1.f), value_buf, value_buf_end);
		RenderText(ImVec2(x_pos - 1.f, frame_bb.Min.y - 1.f), value_buf, value_buf_end);
		RenderText(ImVec2(x_pos, frame_bb.Min.y - 2.0f), value_buf, value_buf_end);
		RenderText(ImVec2(x_pos, frame_bb.Min.y), value_buf, value_buf_end);
		colors[ImGuiCol_Text] = text_color;
		RenderText(ImVec2(x_pos, frame_bb.Min.y - 1.f), value_buf, value_buf_end);
		ImGui::PushClipRect(cur_clipping.Min, cur_clipping.Max, false);
	}
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
	...
	//window->DC.CursorPos.x += 20.f;

    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 5.f/*label_size.y + style.FramePadding.y*2.0f*/));
    ...
    if (tab_focus_requested || (hovered && g.IO.MouseClicked[0] && ImGui::is_input_allowed))
    {
    ...
    ItemSize(total_bb, style.FramePadding.y);
    const bool value_changed = SliderBehavior(frame_bb/* - ImRect(ImVec2(0,0), ImVec2(0,5))*/, id, v, v_min, v_max, power, decimal_precision, ImGuiSliderFlags_RenderValue);
	...
	//value but fuck this
    //RenderTextClipped(frame_bb.Min + ImVec2(grab_pos, 0), frame_bb.Min + ImVec2(grab_pos/* + w*/, label_size.y)/*frame_bb.Max*/, value_buf, value_buf_end, NULL, ImVec2(0.5f,0.5f));
...
void ImGui::Tabs(const char* items, int* current_item)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return;
	...
	int items_count = 0;
	const char* p = items;       // FIXME-OPT: Avoid computing this, or at least only when combo is open
	while (*p)
	{
		p += strlen(p) + 1;
		items_count++;
	...
	auto size = (GetContentRegionMax().x - window->WindowPadding.x - items_count) / items_count;

	for (int i = 0; i < items_count; i++)
	{
		const char* text = NULL;
		Items_SingleStringGetter((void*)items, i, &text);
		if (ImGui::Button(text, ImVec2(size, 20), *current_item == i))
			*current_item = i;

		ImGui::SameLine();
		ImGui::SetCursorPosX(ImGui::GetCursorPosX() - style.ItemSpacing.x + 1);
	}
	ImGui::NewLine();
}
...
std::unordered_map <ImGuiID, std::string> selected_items;

bool ImGui::MCombo(const char* label, std::vector<ImGui_MultiSelectable> items, int height_in_items, ImVec2 size)
{
	ImGuiWindow* window = GetCurrentWindow();
	ImGuiContext& g = *GImGui;
	if (window->SkipItems)
	...
	if (items_count > 1) {
		for (int i = 0; i < items_count; i++)
		{
			...
			if (i > 0 && selected_items[id].size() > 1 && *items[i - 1].value == false)
				selected_items[id] += ", ";

			selected_items[id] += item.name;

			if ((i + 1) < items_count && selected_items[id].size() > 1 && *items[i + 1].value)
				selected_items[id] += ", ";
			/*if (CalcTextSize(selected_items.c_str()).x > 150) {
				selected_items.resize(150 / 5);
				selected_items += "...";
				break;
			}*/
		}
	...
	if (height_in_items != -1 && !g.NextWindowData.SizeConstraintCond)
	{
		float popup_max_height = CalcMaxPopupHeightFromItemCount(height_in_items);
		SetNextWindowSizeConstraints(ImVec2(0, 0), ImVec2(FLT_MAX, popup_max_height));
	}

	if (!BeginCombo(label, selected_items[id].c_str(), 0))
		return false;
	...
	for (int i = 0; i < items_count; i++)
	{
		...
		PushID((void*)(intptr_t)i);
		if (Selectable(item.name.c_str(), *item.value, ImGuiSelectableFlags_DontClosePopups))
		{
			value_changed = true;
			*item.value ^= 1;
		}
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
	...
	/*if (flags & ImGuiColorEditFlags_ColorPicker)
		window->DC.CursorPos.y += 5;*/

    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + size);
    ItemSize(bb, (size.y >= default_size) ? g.Style.FramePadding.y : 0.0f);

    //if (!(flags & ImGuiColorEditFlags_ColorPickerSameline)) {
        if (!ItemAdd(bb, id))
            return false;
    //}
...
bool ImGui::ColorPicker(const char* label, float* col, ImGuiColorEditFlags extra_flags)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	ImDrawList* draw_list = window->DrawList;
	...
	bool alpha_bar = (extra_flags & ImGuiColorEditFlags_AlphaBar) && !(extra_flags & ImGuiColorEditFlags_NoAlpha);

	ImVec2 picker_pos = ImGui::GetCursorScreenPos();
	float square_sz = SmallSquareSize();
	float bars_width = square_sz; // Arbitrary smallish width of Hue/Alpha picking bars
								  //float sv_picker_size = ImMax(bars_width * 1, CalcItemWidth() - (alpha_bar ? 2 : 1) * (bars_width + style.ItemInnerSpacing.x)); // Saturation/Value picking box
	float bar0_pos_x = picker_pos.x + SV_PICKER_SIZE.x + style.ItemInnerSpacing.x + 5;
	float bar1_pos_x = bar0_pos_x + bars_width + style.ItemInnerSpacing.x;
	float bars_triangles_half_sz = (float)(int)(bars_width * 0.20f);
	...
	{
		SetCursorScreenPos(ImVec2(bar1_pos_x, picker_pos.y));
		InvisibleButton("alpha", ImVec2(bars_width, SV_PICKER_SIZE.x));
		if (IsItemActive())
		{
			color.Value.w = 1.0f - ImSaturate((io.MousePos.y - picker_pos.y) / (SV_PICKER_SIZE.x - 1));
			value_changed = true;
		}
	}
	...
	if (alpha_bar)
	{
		float alpha = ImSaturate(color.Value.w);
		ImRect bar1_bb(bar1_pos_x, picker_pos.y, bar1_pos_x + bars_width, picker_pos.y + SV_PICKER_SIZE.x);
		RenderColorRectWithAlphaCheckerboard(bar1_bb.Min, bar1_bb.Max, IM_COL32(0, 0, 0, 0), bar1_bb.GetWidth() / 2.0f, ImVec2(0.0f, 0.0f));
		draw_list->AddRectFilledMultiColor(bar1_bb.Min, bar1_bb.Max, col32_no_alpha, col32_no_alpha, col32_no_alpha & ~IM_COL32_A_MASK, col32_no_alpha & ~IM_COL32_A_MASK);
		float bar1_line_y = (float)(int)(picker_pos.y + (1.0f - alpha) * SV_PICKER_SIZE.x + 0.5f);
		RenderFrameBorder(bar1_bb.Min, bar1_bb.Max, 0.0f);
		RenderArrowsForVerticalBar(draw_list, ImVec2(bar1_pos_x - 1, bar1_line_y), ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz), bars_width + 2.0f);
	}
