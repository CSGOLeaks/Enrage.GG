...
struct IMGUI_API ImGui_MultiSelectable
{
	std::string		name;
	bool*			value;

	ImGui_MultiSelectable(const std::string& name_, bool *value_)
	{
		name = name_;
		value = value_;
	}
...
namespace ImGui
{
	...
	IMGUI_API bool ColorPicker(const char * label, float * col, ImGuiColorEditFlags extra_flags);
    ...
    IMGUI_API void          End();
	IMGUI_API bool BeginGroupBox(const char * name = "", const ImVec2 & size_arg = ImVec2(0,0));
    ...
    IMGUI_API ImU32         GetColorU32(ImGuiCol idx, float alpha_mul = 1.0f, float alpha_replace = -1.f);                  // retrieve given style color with style alpha applied and optional extra alpha multiplier
	...
	IMGUI_API void			Tabs(const char* items, int* current_item);
	...
	bool MCombo(const char * label, std::vector<ImGui_MultiSelectable> items, int height_in_items = -1, ImVec2 size = ImVec2(0, 0));
...
enum ImGuiWindowFlags_
{
	...
	ImGuiWindowFlags_CoolStyle				= 1 << 29,
	ImGuiWindowFlags_GroupBox				= 1 << 30   // Don't use! For internal use by BeginGroupBox()
};
...
enum ImGuiColorEditFlags_
{
	...
	ImGuiColorEditFlags_ColorPicker		= 1 << 20,
	ImGuiColorEditFlags_ColorPickerSameline		= 1 << 21,
    // Internals/Masks
...
struct ImDrawCmd
{
	...
	ImDrawCmd() { ElemCount = 0; ClipRect.x = ClipRect.y = ClipRect.z = ClipRect.w = 0.0f; TextureId = NULL; ShaderID = NULL; UserCallback = NULL; UserCallbackData = NULL; }
};
...
struct ImDrawList
{
	...
	ImVector<ImShaderID>    _ShaderIdStack;     // [Internal]
	...
	IMGUI_API void  PushShaderID(const ImShaderID& shader_id);
	IMGUI_API void  PopTextureID();
	IMGUI_API void  PopShaderID();
